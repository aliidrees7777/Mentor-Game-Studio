  ...
  for (var i = 0; i < selectors.length; i++) {
    ...
    for (var j = 0; j < elements.length; j++) {
      this.elements.push({
        ...
        count: selectors[i].count,
        type: selectors[i].type,
        property: selectors[i].property
      });
    }
  }
};
...
VHChromeFix.prototype.fixAll = function() {
  for (var i = 0; i < this.elements.length; i++) {
    ...
    switch(element.type) {
      case 'vh':
            multiple = window.innerHeight;
            break;
      case 'vmax':
            multiple = window.innerHeight > window.innerWidth ? window.innerHeight : window.innerWidth;
            break;
      default:
            multiple = null;
    }
    element.domElement.style[element.property] = (multiple * element.count / 100) + 'px';
  }
};
